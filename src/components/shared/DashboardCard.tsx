...
const DashboardCardComponent = ({
  ...
  const cardContent = (
    <Card className={cn("glass-card border-zinc-200 shadow-xl overflow-hidden relative group/card h-full transition-all duration-500", cardVariants({ variant }))}>
      ...
      <CardHeader className="relative z-10 flex flex-row items-start justify-between space-y-0 pb-4">
        ...
        <div className="h-8 w-8 rounded-lg bg-zinc-100 border border-zinc-200 flex items-center justify-center group-hover/card:border-primary/30 transition-all">
          <Icon className="h-4 w-4 text-zinc-500 group-hover/card:text-primary transition-colors drop-shadow-[0_0_8px_rgba(59,130,246,0)] group-hover/card:drop-shadow-[0_0_8px_rgba(59,130,246,0.2)]" />
        </div>
      </CardHeader>
      {value !== undefined && (
      ...
      {links && (
        <CardContent className="relative z-10 flex-grow flex flex-col justify-center pt-2 gap-2">
          {links.map((link) => (
            <Link key={link.href} href={link.href} className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 border border-zinc-100 hover:bg-zinc-100 hover:border-zinc-200 transition-all group/link">
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 group-hover/link:text-zinc-900">{link.title}</span>
              <ChevronRight className="h-3 w-3 text-zinc-300 group-hover/link:text-primary transition-colors" />
            </Link>
          ))}
        </CardContent>
      )}
