...
import { ArrowLeft, Loader2, Building2, Smartphone, Coins, FileText, UserCog, ShieldCheck } from 'lucide-react';
...
export default function TokenSettingsPage() {
  ...
  const currencySymbols: { [key: string]: string } = {
    INR: 'â‚¹', USD: '$', EUR: 'â‚¬', GBP: 'Â£', JPY: 'Â¥', AUD: 'A$', CAD: 'C$',
  };
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormField control={form.control} name="otp_request_limit_hourly" render={({ field }) => (
                    <FormItem><FormLabel>OTP Request Limit (per hour)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormDescription>Max number of OTPs a user can request in one hour.</FormDescription></FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="company_state_billing" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value || ""}>
                        ...
                        <SelectContent>{states?.map((s) => (<SelectItem key={s.id} value={s.name} className="capitalize">{s.name}</SelectItem>))}</SelectContent>
                      </Select>
