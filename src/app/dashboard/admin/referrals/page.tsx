    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-10 min-h-screen">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-4xl font-headline font-bold text-white tracking-tighter">
                Agent <span className="text-primary/80">Protocol</span>
              </h1>
            </div>
            <p className="text-zinc-500 text-[11px] font-medium uppercase tracking-[0.2em] ml-1">
              Neural network expansion metrics and commission distribution ledger.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 bg-white/[0.02] border border-white/5 p-3 rounded-2xl backdrop-blur-sm">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-black/40 border border-white/5">
            {settings?.referral_enabled
              ? <><div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /><span className="text-[9px] text-emerald-500 font-black uppercase tracking-widest">Growth Active</span></>
              : <><div className="h-2 w-2 rounded-full bg-zinc-700" /><span className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">Protocol Offline</span></>
            }
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { label: 'Active Referrers', value: totalActiveReferrers, icon: Users, sub: 'Neural Nodes' },
          { label: 'Protocol Events', value: events.length, icon: TrendingUp, sub: 'Ledger Entries' },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-4 space-y-6">
          <div className="flex items-center gap-2 px-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Elite Operatives</span>
          </div>
          <Card className="glass-card border-white/5 bg-black/40 overflow-hidden">
            <CardContent className="p-0">
              {topReferrers.length === 0 ? (
                <div className="py-20 text-center">
                  <p className="text-zinc-700 text-[10px] font-black uppercase tracking-widest">No Active Vectors</p>
                </div>
              ) : (
                  ...
                  {topReferrers.map((r, i) => (
                    <div key={i} className="group flex items-center justify-between px-6 py-5 hover:bg-white/[0.02] transition-colors">
                      <div className="space-y-1">
                        ...
                        <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">{r.count} SUCCESSFUL LINKS</p>
                      </div>
                      <div className="text-right">
                        ...
                        <p className="text-[8px] font-black text-zinc-700 uppercase tracking-tighter mt-0.5">Yield</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center gap-2 px-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Transaction Stream</span>
          </div>
          <Card className="glass-card border-white/5 bg-black/40 overflow-hidden">
            <CardContent className="p-0">
              {isLoading ? (
                <div className="py-20 flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Decrypting Stream...</p>
                </div>
              ) : events.length === 0 ? (
                <div className="py-32 text-center">
                  <p className="text-zinc-700 text-[10px] font-black uppercase tracking-widest">No Stream Data Available</p>
                </div>
              ) : (
                <Table>
                  <TableHeader className="bg-white/[0.02]">
                    <TableRow className="border-white/5 hover:bg-transparent">
                      <TableHead className="text-[9px] font-black uppercase tracking-widest text-zinc-500 h-12 px-6">Source</TableHead>
                      <TableHead className="text-[9px] font-black uppercase tracking-widest text-zinc-500 h-12 px-6">Destination</TableHead>
                      <TableHead className="text-[9px] font-black uppercase tracking-widest text-zinc-500 h-12 px-6">Volume</TableHead>
                      <TableHead className="text-[9px] font-black uppercase tracking-widest text-zinc-500 h-12 px-6">Commission</TableHead>
                      <TableHead className="text-right text-[9px] font-black uppercase tracking-widest text-zinc-500 h-12 px-6">Timestamp</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
