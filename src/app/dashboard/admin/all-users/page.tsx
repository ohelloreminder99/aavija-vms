...
export default function AllUsersPage() {
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative group/search flex-1">
            ...
            <Input
              ...
              className="pl-12 bg-white/20 border-white/10 text-white h-12 rounded-2xl placeholder:text-zinc-400 focus:border-primary/30 transition-all font-semibold"
              value={searchTerm}
              onChange={(e) => {
          ...
          <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-2xl px-4 py-2 self-end md:self-auto">
            <Button
              variant="ghost"
              size="sm"
          ...
          <Table>
            <TableHeader className="bg-white/5">
              <TableRow className="border-white/10 hover:bg-transparent">
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-400 py-6 pl-8">Global Identity</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-400 py-6">Architectural Role</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-400 py-6">Linked Analog</TableHead>
            ...
            <TableBody>
              {filteredUsers.map((user: WithId<UserProfile>) => {
                ...
                return (
                  <TableRow key={user.id} className="border-white/10 hover:bg-white/5 group/row transition-colors">
                    <TableCell className="pl-8 py-5">
                      <div className="flex items-center gap-4">
                        <Avatar className="h-10 w-10 border border-white/10 group-hover/row:border-primary/30 transition-colors shadow-lg">
                          ...
                          <AvatarFallback className="bg-white/10 text-zinc-400 font-bold">{user.name.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col">
                          <span className="font-bold text-white tracking-tight group-hover/row:text-primary transition-colors">{user.name}</span>
                          <span className="text-[10px] text-zinc-400 font-semibold tracking-tight uppercase">
                            {user.email}
                          </span>
                        </div>
                      </div>
                    ...
                    <TableCell>
                      {user.phone ? (
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] font-mono text-zinc-400 font-semibold">{user.phone}</span>
                          {user.is_verified && (
                    ...
                    <TableCell className="text-right pr-8">
                      <Button
                        ...
                        disabled={!user.phone}
                        className="h-9 w-9 rounded-lg bg-white/5 border border-white/10 text-zinc-400 hover:text-red-500 hover:bg-red-500/10 transition-all disabled:opacity-20 shadow-sm"
        ...
        {filteredUsers.length === 0 && (
          <div className="py-20 text-center">
            <p className="text-[11px] font-black text-zinc-300 uppercase tracking-[0.3em]">No matching identity in registry</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="container py-10 max-w-7xl">
      <div className="mb-8">
        <Button asChild variant="ghost" className="text-zinc-400 hover:text-primary hover:bg-white/10 group/back">
      ...
      <Card className="glass-card border-white/10 shadow-2xl relative overflow-hidden mb-20">
        ...
        <CardHeader className="relative z-10 border-b border-white/10 pb-8">
          ...
          <CardDescription className="text-zinc-400 text-[11px] font-semibold uppercase tracking-widest max-w-2xl leading-relaxed">
      ...
      <AlertDialog open={!!userToManage} onOpenChange={(open) => !open && setUserToManage(null)}>
        <AlertDialogContent className="bg-[#010a05]/95 backdrop-blur-3xl border-white/10 shadow-2xl">
          ...
          <AlertDialogFooter className="gap-3 pt-6">
            <AlertDialogCancel className="bg-transparent border-white/10 text-zinc-400 hover:text-white hover:bg-white/10">Cancel</AlertDialogCancel>
